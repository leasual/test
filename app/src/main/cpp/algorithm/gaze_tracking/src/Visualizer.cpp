...
void Visualizer::SetObservationLandmarks1(cv::Mat & rgbframe,
										  const cv::Mat_<float>& landmarks_2D, const cv::Mat_<int>& visibilities)
{
	int n = landmarks_2D.rows / 2;
	for (int i = 0; i < n; ++i)
	{
		if (visibilities.empty() || visibilities.at<int>(i))
		{
			cv::Point featurePoint(cvRound(landmarks_2D.at<float>(i) * (float)draw_multiplier),
								   cvRound(landmarks_2D.at<float>(i + n) * (float)draw_multiplier));
			// A rough heuristic for drawn point size
			int thickness = (int)std::ceil(3.0* ((double)rgbframe.cols) / 640.0);
			int thickness_2 = (int)std::ceil(1.0* ((double)rgbframe.cols) / 640.0);

			cv::circle(rgbframe, featurePoint, 1 * draw_multiplier, cv::Scalar(0, 0, 255), thickness, CV_AA, draw_shiftbits);
			cv::circle(rgbframe, featurePoint, 1 * draw_multiplier, cv::Scalar(255, 0, 0), thickness_2, CV_AA, draw_shiftbits);
		}
		else
		{
			// Draw a fainter point if the landmark is self occluded
			...
			// A rough heuristic for drawn point size
			int thickness = (int)std::ceil(2.5* ((double)rgbframe.cols) / 640.0);
			int thickness_2 = (int)std::ceil(1.0* ((double)rgbframe.cols) / 640.0);

			cv::circle(rgbframe, featurePoint, 1 * draw_multiplier, cv::Scalar(0, 0, 155), thickness, CV_AA, draw_shiftbits);
			cv::circle(rgbframe, featurePoint, 1 * draw_multiplier, cv::Scalar(155, 0, 0), thickness_2, CV_AA, draw_shiftbits);
		}
	}
}
...
void Visualizer::SetGazeLandmarks(
...
//cv::circle(captured_image, featurePoint, 1 * draw_multiplier, cv::Scalar(0, 0, 255), thickness, CV_AA, draw_shiftbits);
			...
			int thickness_2 = (int)std::ceil(1.0* ((double)captured_image.cols) / 640.0);
			cv::circle(captured_image, featurePoint, 1 * draw_multiplier, cv::Scalar
					(0, 255, 0), thickness_2, CV_AA, draw_shiftbits);
		}
	}
}
...
void Visualizer::SetEyeLandmarks(
		const cv::Mat_<float>& landmarks_2D, double confidence)
{
...
int thickness_2 = (int)std::ceil(1.0* ((double)captured_image.cols) / 640.0);
cv::circle(captured_image, featurePoint, 1 * draw_multiplier, cv::Scalar(0, 255, 0),
		   thickness_2, CV_AA, draw_shiftbits);
		}
	}
}
...
void Visualizer::SetObservationPose1(cv::Mat & rgbframe,const cv::Vec6f& pose)
{
	// Only draw if the reliability is reasonable, the value is slightly ad-hoc

	double vis_certainty = 1;

	// Scale from 0 to 1, to allow to indicated by colour how confident we are in the tracking
	vis_certainty = (vis_certainty - visualisation_boundary) / (1 - visualisation_boundary);

	// A rough heuristic for box around the face width
	int thickness = (int)std::ceil(2.0* ((double)rgbframe.cols) / 640.0);

	// Draw it in reddish if uncertain, blueish if certain
	DrawBox(rgbframe, pose, cv::Scalar(vis_certainty*255.0, 0, (1 - vis_certainty) * 255), thickness, fx, fy, cx, cy);
...
// Eye gaze infomration drawing, first of eye landmarks then of gaze
void Visualizer::SetObservationGaze1(cv::Mat & rgbframe,const cv::Point3f&
gaze_direction0, const cv::Point3f& gaze_direction1,
									 ...
									 const std::vector<cv::Point3f>& eye_landmarks3d)
{
	if (eye_landmarks2d.size() > 0)
	{
		...
		for (size_t i = 0; i < eye_landmarks2d.size(); ++i)
		{
			cv::Point featurePoint(cvRound(eye_landmarks2d[i].x * (double)draw_multiplier),
			...
			cv::Point nextFeaturePoint(cvRound(eye_landmarks2d[next_point].x * (double)draw_multiplier),
									   cvRound(eye_landmarks2d[next_point].y * (double)draw_multiplier));
			if ((i < 28 && (i < 8 || i > 19)) || (i >= 28 && (i < 8 + 28 || i > 19 + 28)))
				cv::line(rgbframe, featurePoint, nextFeaturePoint, cv::Scalar(255, 0, 0), thickness_2, CV_AA, draw_shiftbits);
			else
				cv::line(rgbframe, featurePoint, nextFeaturePoint, cv::Scalar(0, 0, 255), thickness_2, CV_AA, draw_shiftbits);
		...
		cv::Mat_<float> mesh_0 = (cv::Mat_<float>(2, 3) << points_left[0].x, points_left[0].y, points_left[0].z, points_left[1].x, points_left[1].y, points_left[1].z);
		Project(proj_points, mesh_0, fx, fy, cx, cy);
		cv::line(rgbframe, cv::Point(cvRound(proj_points.at<float>(0, 0) * (float)draw_multiplier), cvRound(proj_points.at<float>(0, 1) * (float)draw_multiplier)),
				 cv::Point(cvRound(proj_points.at<float>(1, 0) * (float)draw_multiplier), cvRound(proj_points.at<float>(1, 1) * (float)draw_multiplier)), cv::Scalar(110, 220, 0), 2, CV_AA, draw_shiftbits);
