...
unsigned char sw2001_rand(void)
{
  ...
  seed = seed + get_random_int(); // rand(); <------------------ add time value
...
/*-------------------------------------------------------------------------*/
static void print_client(struct i2c_client * client)
{
	printk("---client->flags: %d---\n",client->flags);
	printk("---client->addr: 0x%x---\n",client->addr);	
	printk("---client->name: %s---\n",client->name);	
	printk("---client->adapter->name: %s---\n",client->adapter->name);	
	printk("---client->adapter->algo: 0x%x---\n",(unsigned int)client->adapter->algo);		
	printk("---client->adapter->algo->smbus_xfer: 0x%x---\n",(unsigned int)client->adapter->algo->smbus_xfer);
	printk("---client->adapter->algo->master_xfer: 0x%x---\n",(unsigned int)client->adapter->algo->master_xfer);		
}
