...
bool CLNF::Read_CLNF(string clnf_location) {
    ...
    while (!locations.eof()) {
        ...
        if (module.compare("PDM") == 0) {
            ...
            int TotalN = pdm.mean_shape.rows * pdm.mean_shape.cols;
            patch_experts._TtLandMarkNumEPX_ = TotalN / 3;
    ...
    bool read_success = patch_experts.Read11(intensity_expert_locations,
                                             ccnf_expert_locations, cen_expert_locations, early_term_loc);
...
bool CLNF::DetectLandmarks(const cv::Mat_<uchar> &image, FaceModelParameters &params) {
    ...
    pdm.CalcShape2D(detected_landmarks,
...
bool CLNF::DetectLandmarksfx(const cv::Mat_<uchar> &image,
                             FaceModelParameters &params, bool useclnf) {
    ...
    for (int part_model = 0; part_model < hierarchical_models.size(); part_model++) {
        cv::Mat_<float> part_model_locs;
        UpdatePartsByEntire(part_model, part_model_locs);
    ...
    double duration11 = (t2 - t1) / cv::getTickFrequency();
//    cout << "duration:" << duration11 << endl;
...
int CLNF::UpdatePartsByEntire(int part_model, cv::Mat_<float> &part_locs) {
    ...
    part_locs.create(cv::Size(n_part_points * 2, 1));
    part_locs.setTo(0.0);

    for (size_t mapping_ind = 0; mapping_ind < mappings.size(); ++mapping_ind) {
        part_locs.at<float>(
                mappings[mapping_ind].second) = detected_landmarks.at<float>(
                mappings[mapping_ind].first);
        part_locs.at<float>(mappings[mapping_ind].second + n_part_points) =
                detected_landmarks.at<float>(
                        mappings[mapping_ind].first + this->pdm.NumberOfPoints());
    }

    return 0;
}

void CLNF::readctviews(const std::string &model_dir) {
    cv::FileStorage fs(model_dir + "/Centerviews.xml", cv::FileStorage::READ);
    int scalenumber = 0, viewnumber = 0;
    fs["scalenumber"] >> scalenumber;
    char viewstorage[100];
    sprintf(viewstorage, "viewnumber%02d", (scalenumber - 1));
    printf("viewstorage:%s\n", viewstorage);
    fs[viewstorage] >> viewnumber;
    centers.resize(viewnumber);
    visibilities.resize(viewnumber);
    for (size_t vidx = 0; vidx < viewnumber; vidx++) {
        char centername[260], viewname[260];
        sprintf(centername, "CENTER%02dscale%02dview", int(scalenumber - 1), int(vidx));
        sprintf(viewname, "VIEW%02dscale%02dview", int(scalenumber - 1), int(vidx));
        fs[centername] >> centers[vidx];
        fs[viewname] >> visibilities[vidx];
    }
}

int CLNF::updateEntireByParts() {
    ...
    for (size_t part_model = 0; part_model < hierarchical_models.size(); ++part_model) {
        ...
        for (size_t mapping_ind = 0; mapping_ind < mappings.size(); ++mapping_ind) {
            detected_landmarks.at<float>(
                    mappings[mapping_ind].first) = hierarchical_models[part_model].detected_landmarks.at<float>(
                    mappings[mapping_ind].second);
            detected_landmarks.at<float>(mappings[mapping_ind].first + pdm.NumberOfPoints())
                    = hierarchical_models[part_model].detected_landmarks.at<float>(mappings[mapping_ind].second +
...
bool CLNF::Fit(const cv::Mat_<float> &im, const std::vector<int> &window_sizes, const FaceModelParameters &parameters) {
    ...
    for (int scale = 0; scale < num_scales; scale++) {
        ...
        patch_experts.Response(patch_expert_responses, sim_ref_to_img, sim_img_to_ref, im, pdm, params_global,
                               params_local, window_size, scale);
        ...
        if (parameters.refine_parameters == true) {
            ...
            tmp_parameters.reg_factor =
                    parameters.reg_factor - 15 * log(patch_experts.patch_scaling[scale_max] / 0.25) / log(2);

            if (tmp_parameters.reg_factor <= 0)
                tmp_parameters.reg_factor = 0.001;

            tmp_parameters.sigma =
                    parameters.sigma + 0.25 * log(patch_experts.patch_scaling[scale_max] / 0.25) / log(2);
            tmp_parameters.weight_factor = parameters.weight_factor + 2 * parameters.weight_factor *
                                                                      log(patch_experts.patch_scaling[scale_max] /
                                                                          0.25) / log(2);
        }
        ...
        this->NU_RLMS(params_global, params_local, patch_expert_responses, cv::Vec6f(params_global),
                      params_local.clone(), current_shape, sim_img_to_ref, sim_ref_to_img, window_size, view_id, true,
        ...
        if (scale == num_scales - 1 || window_sizes[scale + 1] == 0 || params_global[0] < 0.30) {
            this->model_likelihood = this->NU_RLMS(params_global, params_local, patch_expert_responses,
            ...
            this->NU_RLMS(params_global, params_local, patch_expert_responses, cv::Vec6f(params_global),
                          params_local.clone(), current_shape, sim_img_to_ref, sim_ref_to_img, window_size, view_id,
                          false, scale, this->landmark_likelihoods, tmp_parameters, false);
        }
...
void CLNF::NonVectorisedMeanShift_precalc_kde(cv::Mat_<float> &out_mean_shifts,
    ...
    for (int i = 0; i < n; i++) {
        ...
        int idx = closest_row * ((int) (resp_size / step_size + 0.5)) +
...
void
CLNF::GetWeightMatrix(cv::Mat_<float> &WeightMatrix, int scale, int view_id, const FaceModelParameters &parameters) {
    int n = pdm.NumberOfPoints();
    ...
    if (parameters.weight_factor > 0) {
        ...
        for (int p = 0; p < n; p++) {
            if (!patch_experts.ccnf_expert_intensity.empty()) {
                ...
                // for the x dimension
                WeightMatrix.at<float>(p, p) = WeightMatrix.at<float>(p, p) +
                                               patch_experts.ccnf_expert_intensity[scale][view_id][p].patch_confidence;
...
float CLNF::NU_RLMS(cv::Vec6f &final_global, cv::Mat_<float> &final_local,
                    ...
                    const cv::Matx22f &sim_ref_to_img, int resp_size, int view_id, bool rigid, int scale,
    ...
    for (int iter = 0; iter < parameters.num_optimisation_iteration; iter++) {
        ...
        NonVectorisedMeanShift_precalc_kde(mean_shifts, patch_expert_responses, dxs, dys, resp_size, a, scale, view_id,
                                           kde_resp_precalc);
        ...
        if (!rigid) {
            J_w_t_m(cv::Rect(0, 6, 1, m)) =
                    J_w_t_m(cv::Rect(0, 6, 1, m)) - regTerm(cv::Rect(6, 6, m, m)) * current_local;
        }
        ...
        char N[2];
        N[0] = 'N';
        sgemm_(N, N, &J.cols, &J_w_t.rows, &J_w_t.cols, &alpha1, (float *) J.data, &J.cols, (float *) J_w_t.data,
               &J_w_t.cols, &beta1, (float *) Hessian.data, &J.cols);
...
cv::Mat_<float> CLNF::GetShape(float fx, float fy, float cx, float cy) const {
    ...
    cv::Matx33f R = Utilities::Euler2RotationMatrix(
            cv::Vec3f((float) params_global[1], (float) params_global[2], (float) params_global[3]));
...
cv::Mat_<int> CLNF::GetVisibilities11() const {
    int bstview = 0;
    float dbest;
    for (int i = 0; i < int(centers.size()); i++) {
        float v1 = params_global[1] - centers[i][0];
        float v2 = params_global[2] - centers[i][1];
        float v3 = params_global[3] - centers[i][2];
    ...
    cv::Mat_<int> visibilities_to_ret = this->visibilities[bstview].clone();
    return visibilities_to_ret;
}
